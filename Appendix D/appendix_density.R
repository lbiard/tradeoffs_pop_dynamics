...
p <- ggplot(trial_plot, aes(x = (x2.sim), y = bayes.c.eff.mean))+
  ...
  ggtitle("")+
  xlab("Log baseline generation time")+
  ylab("% change CV growth rate")+
  theme_bw() +
  theme(legend.position = "none")+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  theme(axis.text=element_text(size=14),
        axis.title=element_text(size=16))
p <- p + geom_point(data = point_plot, aes(y=point_plot, x=log(V2)), position = position_jitter(w = 0.1, h = 0),alpha=0.02, color="#E1AF00", size=1)
p <- p + geom_line(data=plot.dat,aes(x = (x2.sim), y = bayes.c.eff.mean),color = "#E1AF00", alpha = 0.8, size = 1.2)+
  geom_ribbon(data=plot.dat,aes(ymin = bayes.c.eff.lower, ymax = bayes.c.eff.upper), fill = "#E1AF00", alpha = 0.2)
p <- p + geom_point(data = trial_plot, aes(x=log(gentime), y=V1), color="#E1AF00", size=1.5, alpha = 0.5)

p <- p + geom_point(data = point_plot_2, aes(y=point_plot_2, x=log(V2)), position = position_jitter(w = 0.1, h = 0),alpha=0.02, color="#F21A00", size=1)
p <- p + geom_line(data=plot.dat_2,aes(x = (x2.sim), y = bayes.c.eff.mean_2),color = "#F21A00", alpha = 0.8, size = 1.2)+
  geom_ribbon(data=plot.dat_2,aes(ymin = bayes.c.eff.lower_2, ymax = bayes.c.eff.upper_2), fill = "#F21A00", alpha = 0.2)
p <- p + geom_point(data = trial_plot_2, aes(x=log(gentime), y=V1), color="#F21A00", size=1.5, alpha = 0.5)
