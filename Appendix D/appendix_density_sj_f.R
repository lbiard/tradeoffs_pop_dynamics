...
context_dependent_cor <- function(inds, t, climate){
  ...
  mk_rescale_matrix <- function(eigen_vals, env) {
    x <- sqrt(eigen_vals[1]) - 1
    y <- (1 - sqrt(eigen_vals[2])) / sqrt(eigen_vals[2])
    diag(c(1 / (1 + x * env), 1 + y * env))
  }
  ...
  rescale_mat <- mk_rescale_matrix(eigen_system$values, env) 
...
simulate_mean_vr <- function(inds, t, climate, X2, lh, lower_level_elasticities, scenario){
  ...
  gamma_survival_ju <- X2[,1] * sd_sj  
  gamma_transition <- inds$quality_ta * sd_ta
  gamma_survival_ad <- inds$quality_sa * sd_sa        # rate involved in the tradeoff
  ...
  slope_dens_survival_ju <- NULL
  slope_dens_survival_ju[1] <- -0.0002         # baseline
  slope_dens_survival_ju[2] <- -0.0001         # -50% 
  slope_dens_survival_ju[3] <- -0.00005        # -75%
  ...
  slope_dens_transition <- NULL
  slope_dens_transition[1] <- -0.0002         # baseline 
  slope_dens_transition[2] <- -0.0001         # -50% 
  slope_dens_transition[3] <- -0.00005        # -75%
  ...
  slope_dens_survival_ad <- NULL
  slope_dens_survival_ad[1] <- -0.0002         # baseline
  slope_dens_survival_ad[2] <- -0.0001         # -50% 
  slope_dens_survival_ad[3] <- -0.00005        # -75%
  ...
  slope_dens_fecundity_ad <- NULL
  slope_dens_fecundity_ad[1] <- -0.0002         # baseline
  slope_dens_fecundity_ad[2] <- -0.0001        # -50% 
  slope_dens_fecundity_ad[3] <- -0.00005       # -75%
  slope_env_fecundity_ad <- 1-lower_level_elasticities[4]    # buffering hypothesis 
  
  
  ## Survival juvenile GLM ##
  survival_ju <- function(env, dens, scenario) {
    
    exp(intercept_survival_ju + slope_env_survival_ju*logit_env + slope_dens_survival_ju[scenario]*dens + gamma_survival_ju)/
      (1+exp(intercept_survival_ju + slope_env_survival_ju*logit_env + slope_dens_survival_ju[scenario]*dens + gamma_survival_ju))
    
  }
  
  mu_sj <- survival_ju(env = logit_env, dens = dens, scenario = scenario)
  # mu_sj contain juvenile survival rate for each individual at the given time step  
  
  ## Maturation GLM ##
  transition_ad <- function(env, dens, scenario) {
    
    exp(intercept_transition + slope_env_transition*logit_env + slope_dens_transition[scenario]*dens + gamma_transition)/
      (1+exp(intercept_transition + slope_env_transition*logit_env + slope_dens_transition[scenario]*dens + gamma_transition))
    
  }
  
  mu_ta <- transition_ad(env = logit_env, dens = dens, scenario = scenario)
  # mu_ta contain maturation rate for each individual at the given time step  
  
  ## Survival adult GLM ##
  survival_ad <- function(env, dens, scenario) {
    
    exp(intercept_survival_ad + slope_env_survival_ad*logit_env + slope_dens_survival_ad[scenario]*dens + gamma_survival_ad)/
      (1+exp(intercept_survival_ad + slope_env_survival_ad*logit_env + slope_dens_survival_ad[scenario]*dens + gamma_survival_ad))
    
  }
  
  mu_sa <- survival_ad(env = logit_env, dens = dens, scenario = scenario)
  # mu_sa contain adult survival rate for each individual at the given time step  
  
  ## Fecundity adult GLM ##
  fecundity_ad <- function(env, dens, scenario) {
    
    exp(intercept_fecundity_ad + slope_env_fecundity_ad*logit_env + slope_dens_fecundity_ad[scenario]*dens + gamma_fecundity_ad)
    
  }
  
  mu_fa <- fecundity_ad(env = logit_env, dens = dens, scenario = scenario)
...
for (lh in 1:dim(lhs)[1]) {   ## loop over life histories
  ...
  for (scenario in 1:3) {    ## loop over scenarios of context dependence of covariation
    
    for (i in 1:replicat) {    ## loop over replicate of the simulation
      
      # create the environmental variable
      set.seed(i+100) # so that environment is different among replicates, but similar accross life histories
      ...
      while(ts < time_steps && dim(inds[inds$survival==1,])[1]>3){
        ...
        X2                 <- context_dependent_cor(inds = inds, t = ts+1, climate = climate)   # update covariation for the time step
        vr.mu              <- simulate_mean_vr(inds = inds, t = ts+1, climate = climate, X2 = X2, lh = lhs[lh,], 
                                               lower_level_elasticities = lower_level_elasticities[lh,], scenario = scenario)          # simulate vital rates for all individuals
...
## -75% density dependence

# annual growth rate
stochastic_growth_rate_serie_scenario_3 <- (ind_abund[2:(time_steps+1),,3,]/ind_abund[1:time_steps,,3,])
# stochastic growth rate
stochastic_growth_rate_scenario_3 <- apply(stochastic_growth_rate_serie_scenario_3, c(2,3), function(x) geometric.mean(x[x>0]))
# coefficient of variation in annual growth rate
coef_var_sgr_scenario_3 <- apply(stochastic_growth_rate_serie_scenario_3, c(2,3), function(x)  sd(x, na.rm = T)/mean(x, na.rm = T))
...
# Decrease 50% density dependence
change_coef_var_1 <- ((coef_var_sgr_scenario_2 - coef_var_sgr_scenario_1) / coef_var_sgr_scenario_1)*100
# Decrease 75% density dependence
change_coef_var_3 <- ((coef_var_sgr_scenario_3 - coef_var_sgr_scenario_1) / coef_var_sgr_scenario_1)*100



####################################################
####################################################
#### Figure CV growth rate - density dependence ####
####################################################
####################################################

## Decrease density dependence by 50% ##
...
## Decrease density dependence by 75% ##

trial_plot_2 <- as.data.frame(cbind(apply(change_coef_var_3, 2, function(x) mean(x, na.rm=T)),
                                  apply(change_coef_var_3, 2, function(x) quantile(x, probs = c(0.025), na.rm=T)),
                                  apply(change_coef_var_3, 2, function(x) quantile(x, probs = c(0.975), na.rm=T)),
                                  gentime))

point_plot_2 <- t(change_coef_var_3)
point_plot_2 <- c(point_plot_2)
point_plot_2 <- as.data.frame(cbind(point_plot_2,  rep(gentime, replicat), rep((1:125), replicat)))


# change in CV growth rate ~ log(baseline generation time) + (1|life history)
fit_2 <- brm(point_plot_2 ~ log(V2) + (1|V3), data = point_plot_2, 
           cores=2, chains=2, iter = 6000, warmup = 3000)
summary(fit_2)

posterior_2 <- as.matrix(fit_2)
dat_plot_2 <- as.data.frame(posterior_2)


x2.sim <- seq(min(log(gentime)), max(log(gentime)), by = 0.005)

int.sim_2 <- matrix(rep(NA, nrow(dat_plot_2)*length(x2.sim)), nrow = nrow(dat_plot_2))
for(i in 1:length(x2.sim)){
  int.sim_2[, i] <- dat_plot_2$b_Intercept + dat_plot_2$b_logV2 * (x2.sim[i])
}

bayes.c.eff.mean_2 <- apply(int.sim_2, 2, mean)
bayes.c.eff.lower_2 <- apply(int.sim_2, 2, function(x) quantile(x, probs = c(0.025)))
bayes.c.eff.upper_2 <- apply(int.sim_2, 2, function(x) quantile(x, probs = c(0.975)))
plot.dat_2 <- data.frame(x2.sim, bayes.c.eff.mean_2, bayes.c.eff.lower_2, bayes.c.eff.upper_2)


## Plot decrease 50% and 75% together
...
p2 <- p2 + geom_point(data = point_plot, aes(y=point_plot, x=log(V2)), position = position_jitter(w = 0.1, h = 0),alpha=0.02, color="#E1AF00", size=1)
p2 <- p2 + geom_line(data=plot.dat,aes(x = (x2.sim), y = bayes.c.eff.mean),color = "#E1AF00", alpha = 0.8, size = 1.2)+
  geom_ribbon(data=plot.dat,aes(ymin = bayes.c.eff.lower, ymax = bayes.c.eff.upper), fill = "#E1AF00", alpha = 0.2)
p2 <- p2 + geom_point(data = trial_plot, aes(x=log(gentime), y=V1), color="#E1AF00", size=1.5, alpha = 0.5)

p2 <- p2 + geom_point(data = point_plot_2, aes(y=point_plot_2, x=log(V2)), position = position_jitter(w = 0.1, h = 0),alpha=0.02, color="#F21A00", size=1)
p2 <- p2 + geom_line(data=plot.dat_2,aes(x = (x2.sim), y = bayes.c.eff.mean_2),color = "#F21A00", alpha = 0.8, size = 1.2)+
  geom_ribbon(data=plot.dat_2,aes(ymin = bayes.c.eff.lower_2, ymax = bayes.c.eff.upper_2), fill = "#F21A00", alpha = 0.2)
p2 <- p2 + geom_point(data = trial_plot_2, aes(x=log(gentime), y=V1), color="#F21A00", size=1.5, alpha = 0.5)
