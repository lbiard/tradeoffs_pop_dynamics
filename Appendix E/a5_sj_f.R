...
context_dependent_cor <- function(inds, t, climate, scenario){
  ...
  env <- exp(climate[t])/(1+exp(climate[t]))
  
  X <- cbind(inds$quality_sj, inds$quality_f)   # the 2 traits involved in the tradeoff
  
  sd_X1 <- sqrt(var(inds$quality_sj))
  sd_X2 <- sqrt(var(inds$quality_f))
  
  if(dim(inds)[1] > 2) {
    
    # Transforming data to circle/sphere
    cc <- colMeans(X) # computing centre of point cloud
    
    # A note on PCA: X %*% PCA_coefs = PCA_scores
    score = X %*% coef
    # normalise data along each PCA axis by standard deviation
    sd_Xpca = apply(score,2,sd)  # computing standard deviation along each dimension in PCA space
    
    x <- NULL
    y <- NULL
    #scenario maximum context-dependence only
    x[1] <- (sd_Xpca[1]-1)
    y[1] <- (1-sd_Xpca[2])/(sd_Xpca[2])
    x[2] <- 0
    y[2] <- 0
    
    sd_Xpca <- c((1+x[scenario]*env), 1/(1+y[scenario]*env))   # new SD to reshape the cloud of point 
    
    scoreNorm = t(t(score)/sd_Xpca) # normalising data in each dimension by sd of that dimension
    # Transform back to original axes
    # step 1: rotate the data back to original axis directions
    X2 = scoreNorm %*% t(coef)
    
    # step 2: un-centre the data
    X2 = scale(X2, center = -cc, scale = FALSE)
    
    if (sqrt(var(X2)[1,1]) !=0 ){
      X2[,1] <- (X2[,1]/sqrt(var(X2)[1,1])) * sd_X1
    }
    if (sqrt(var(X2)[2,2]) !=0 ){
      X2[,2] <- (X2[,2]/sqrt(var(X2)[2,2])) * sd_X2
    }
  }
...
for (lh in 1:dim(lhs)[1]) {   # loop over life histories
  ...
  for (scenario in 1:2) {    # loop over scenarios of context dependence of covariation
    ...
    for (i in 1:replicat) {    # loop over replicate of the simulation
      ...
      rho <- NULL
      rho[1] <- -0.9    # most negative correlation is -0.9
      rho[2] <- -0.5
      
      correlation_matrix <- matrix(c(1, 0, 0, rho[scenario],
                                     0, 1, 0, 0,
                                     0, 0, 1, 0,
                                     rho[scenario], 0, 0, 1),
                                   ncol = 4, nrow = 4)
      ...
      # get coefficients for the rotation of the point of cloud (ordination)
      coef <- eigen(correlation_matrix[c(1,4),c(1,4)])$vectors * (-1)  
      ...
      while(ts < time_steps && dim(inds[inds$survival==1,])[1]>3){
        ...
        store_cor_x[ts, i, scenario, lh] <- cor(X2[,1],X2[,2])
...
# Removing correlation variance
change_coef_var_1 <- ((coef_var_sgr_scenario_1 - coef_var_sgr_scenario_2) / coef_var_sgr_scenario_2)*100
...
q <- q + geom_point(data = point_plot, aes(y=point_plot, x=log(V2)), position = position_jitter(w = 0.1, h = 0),alpha=0.02, size=1)
q <- q + geom_line(data=plot.dat,aes(x = (x2.sim), y = bayes.c.eff.mean),color = "black", alpha = 0.8, size = 1.2)+
  geom_ribbon(data=plot.dat,aes(ymin = bayes.c.eff.lower, ymax = bayes.c.eff.upper), fill = "black", alpha = 0.2)
q <- q + geom_point(data = trial_plot, aes(x=log(gentime), y=V1), size=1.5)
