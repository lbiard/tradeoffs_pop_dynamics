...
context_dependent_cor <- function(inds, t, climate, scenario){
  ...
  if(dim(inds)[1] > 2) {
    ...
    angle <- NULL
    #scenario 2
    angle[2] <- env*45*(pi/180)
    #scenario 1
    angle[1] <- env*0*(pi/180)
    
    X2 = cbind(X[,1] * cos(angle[scenario]) - X[,2] * sin(angle[scenario]),
               X[,1] * sin(angle[scenario]) + X[,2] * cos(angle[scenario]))
...
for (lh in 1:dim(lhs)[1]) {   # loop over life histories
  ...
  for (scenario in 1:2) {    # loop over scenarios of context dependence of covariation
    ...
    for (i in 1:replicat) {    # loop over replicate of the simulation
      ...
      while(ts < time_steps && dim(inds[inds$survival==1,])[1]!=0 && dim(inds[inds$survival==1,])[1]!=1){
